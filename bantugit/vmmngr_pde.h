...
{//! this format is defined by the i86 architecture-
	...
	extern "C" void pd_entry_add_attrib(pd_entry* e, uint32_t attrib);

	//! clears a flag in the page table entry
	extern "C" void pd_entry_del_attrib(pd_entry* e, uint32_t attrib);

	//! sets a frame to page table entry
	extern "C" void pd_entry_set_frame(pd_entry*, physical_addr);

	//! test if page is present
	extern "C" bool pd_entry_is_present(pd_entry e);

	//! test if directory is user mode
	extern "C" bool pd_entry_is_user(pd_entry);

	//! test if directory contains 4mb pages
	extern "C" bool pd_entry_is_4mb(pd_entry);

	//! test if page is writable
	extern "C" bool pd_entry_is_writable(pd_entry e);

	//! get page table entry frame address
	extern "C" physical_addr pd_entry_pfn(pd_entry e);

	//! enable global pages
	extern "C" void pd_entry_enable_global(pd_entry e);
}
